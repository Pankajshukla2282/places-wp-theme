...
?>
  ...
  <article role="article" id="post_<?php the_ID()?>">
    <header>
      ...
      <h5>
        <em>
          <span class="text-muted author"><?php _e('By', 'pd'); echo " "; the_author() ?>,</span>
    ...
    <section>
      ...
      <?php the_content( __( '&hellip; ' . __('Continue reading', 'pd' ) . ' <i class="fa fa-arrow-right"></i>', 'pd' ) ); ?>
    </section>
    <footer>
      <p class="text-muted" style="margin-bottom: 20px;">
        ...
        <?php _e('Comments', 'pd'); ?>:
        <?php comments_popup_link(__('None', 'pd'), '1', '%'); ?>
      </p>
    </footer>
  </article>
  <?php endwhile; ?>

  <?php if ( function_exists('pd_pagination') ) { pd_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="page-item older">
      <?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'pd')) ?></li>
    <li class="page-item newer">
      <?php previous_posts_link(__('Next', 'pd') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
  <?php } ?>
